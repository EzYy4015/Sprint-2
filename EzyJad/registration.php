...
if($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      //Variables which check phone number format
    ...
    if($password == $cpassword){
        ...
        if(!$uppercase || !$lowercase || !$number || strlen($password) < 8) {
            ...
            //This function here checks on validity of email 
            if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
                echo "<script>alert('Email Invalid.')</script>";
            }
            else{
                //This function here checks on validity of phone number
                if(!$digits){
                    echo "<script>alert('Phone Number Invalid.')</script>";
                }
                else{

                    $checkemail = "SELECT * FROM accounts WHERE accEmail = '$email'";
                    $run = mysqli_query($con, $checkemail);

                    //This function checks whether the email exist in the table, if more than 0, then it will not let the user create the account
                    if (!$run->num_rows > 0){

                        $checkname = "SELECT * FROM accounts WHERE accName = '$name'";
                        $run = mysqli_query($con, $checkname);

                        //This function checks whether the username exist in the table, if more than 0, then it will not let the user create the account
                        if (!$run->num_rows > 0){
                        //The variable sqll inserts the username and password entered by the user into the table located inside the database
                            $sqll = "INSERT INTO accounts (accName,accEmail,accPassword,accPhoneNo,accCountry,accAge)
                              VALUES ('$name', '$email', '$password', '$phone', '$country', '$dob')";
                        ...
                        echo "<script>alert('This email is already registered by an account')</script>";
                    }
                }
            }
        }
...
<HTML>
<HEAD>
    <TITLE>Cacti Succulents - Registration</TITLE>
    <meta charset="utf-8"/>
    ...
    <div class = "split-screen">
        <div class = "left">
            <section class="copy">
                ...
                <p>We sell various type and size of Succulent Plants here!</p>
                <p>Gardening tools, soils, fertilizers are sold in an affordable cost!</p>
            </section>
            </section>
        </div>
        <div class="right">
            <form action='' method="POST">
                ...
                <div class="input-container country">
                    ...
                    <input list="countries" name="country" type="text" required>
